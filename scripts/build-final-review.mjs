...
import {dataUrl, ensureDir, exists, findPlaywright, parseArgs, platformPaths, pngSize, writeJson} from "./lib.mjs";
...
async function inspectWechat() {
  ...
  return {ready: reasons.length === 0, reasons, html, text};
}
...
const payload = Buffer.from(JSON.stringify({xhs, wechat, video}), "utf8").toString("base64");
...
<section class="panel"><div class="bar"><h2>短视频</h2><span class="badge" id="video-badge">—</span></div><div class="video-stage"><video id="video-player" controls preload="metadata"></video><div id="video-empty" class="video-empty">短视频待生成</div></div><div class="video-meta" id="video-meta"></div><div class="sub-box"><h3>字幕</h3><pre id="subtitle-text"></pre></div></section>
...
(function(){const player=document.querySelector('#video-player'),empty=document.querySelector('#video-empty'),badge=document.querySelector('#video-badge'),meta=document.querySelector('#video-meta'),subtitle=document.querySelector('#subtitle-text');if(!data.video){subtitle.textContent='（视频尚未生成，无字幕数据）';return}player.style.display='block';empty.hidden=true;player.src=data.video.src;const report=data.video.report;const temporalPassed=report?.checks?.temporal?.passed===true;badge.textContent=temporalPassed?'候选':'未通过';if(temporalPassed)badge.classList.add('ok');const chips=[];if(report?.probe?.format?.duration)chips.push((Math.round(Number(report.probe.format.duration)*10)/10)+'s');chips.push(temporalPassed?'闪烁检查通过':'闪烁检查缺失或失败');meta.innerHTML=chips.map(value=>'<span class="chip">'+value+'</span>').join('');subtitle.textContent=data.video.subtitles||'（无字幕文件）'})();
document.body.dataset.xhsReady=data.xhs.ready?'1':'0';document.body.dataset.wechatReady=data.wechat.ready?'1':'0';document.body.dataset.hasVideo=data.video?'1':'0';document.body.dataset.initialized='1';
</script></body></html>`;
...
if (video) await page.waitForFunction(() => document.querySelector("#video-player")?.videoWidth > 0 && document.querySelector("#video-player")?.duration > 0);
let before = null;
let after = null;
if (xhs.ready) {
...
const metrics = await page.evaluate(() => ({
  ...
  video: {src: document.querySelector("#video-player").getAttribute("src") || "", width: document.querySelector("#video-player").videoWidth, duration: Number.isFinite(document.querySelector("#video-player").duration) ? document.querySelector("#video-player").duration : 0, subtitleLength: document.querySelector("#subtitle-text").innerText.length},
...
const videoBehavior = !video || (video.report?.checks?.temporal?.passed === true && metrics.video.width > 0 && metrics.video.duration > 0);
const passed = xhs.ready && wechat.ready && xhsBehavior && wechatBehavior && videoBehavior && metrics.overflowX <= 1 && Object.values(errors).every((items) => items.length === 0);
const report = {generatedAt: new Date().toISOString(), sourceMode, components: {xhs: {ready: xhs.ready, reasons: xhs.reasons, pages: xhs.pages.length, behaviorPassed: xhsBehavior}, wechat: {ready: wechat.ready, reasons: wechat.reasons, behaviorPassed: wechatBehavior}, video: video ? {ready: true, src: video.src, sizeBytes: video.sizeBytes, temporalPassed: video.report?.checks?.temporal?.passed ?? null, behaviorPassed: videoBehavior} : {ready: false, behaviorPassed: true}}, carouselChanged: xhs.ready ? before !== after : false, metrics, errors, passed};
