...
<section class="panel"><div class="bar"><h2>短视频</h2><span class="badge" id="video-badge">—</span></div><div class="video-stage"><video id="video-player" controls preload="metadata"></video><img id="video-cover-check" alt="" hidden><div id="video-empty" class="video-empty">短视频待生成</div></div><div class="video-meta" id="video-meta"></div><div class="sub-box"><h3>字幕</h3><pre id="subtitle-text"></pre></div></section>
...
const report = {generatedAt: new Date().toISOString(), sourceMode, components: {xhs: {ready: xhs.ready, reasons: xhs.reasons, pages: xhs.pages.length, behaviorPassed: xhsBehavior}, wechat: {ready: wechat.ready, reasons: wechat.reasons, behaviorPassed: wechatBehavior}, video: video ? {ready: videoBehavior, src: video.src, sizeBytes: video.sizeBytes, coverAssetPresent: Boolean(video.cover), coverPassed: video.report?.checks?.cover?.passed === true && Boolean(video.cover), temporalPassed: video.report?.checks?.temporal?.passed ?? null, behaviorPassed: videoBehavior} : {ready: false, behaviorPassed: true}}, carouselChanged: xhs.ready ? before !== after : false, metrics, errors, passed};
